...
export class StreakWidget {
  ...
  public render(info: StreakInfo): void {
    if (!this.container || !info) return;
    try {
      const targetDaily = info.targetDaily || 10;
      const pct = Math.min(100, Math.round((info.racesDoneToday / targetDaily) * 100));
      const isStreakDone = info.racesDoneToday >= targetDaily;
      ...
      const remainingText = (info.racesRemaining || 0) > 0 ? `${info.racesRemaining} more needed` : "Goal reached!";
      const dayStreakText = `🔥 ${info.currentDayStreak || 0}d streak`;
      ...
      if (streakValEl && progressFillEl && badgeEl && timerEl && bestEl && bestQotdEl && remainingEl && dayStreakEl) {
        ...
        streakValEl.textContent = `${info.racesDoneToday} / ${targetDaily}`;
        ...
        timerEl.textContent = info.formattedCountdown || "00:00:00";
      ...
      this.container.innerHTML = `
        <div class="tr-card">
          ...
          <div class="tr-streak-header">
            ...
            <span class="${valClass}">${info.racesDoneToday} / ${targetDaily}</span>
          </div>
          ...
          <div style="border-top: 1px solid rgba(255, 255, 255, 0.08); margin-top: 8px; padding-top: 8px;">
            ...
            <div class="tr-status-row" style="margin-top: 4px;">
              ...
              <span class="tr-timer" title="Time until UTC reset">${info.formattedCountdown || "00:00:00"}</span>
            </div>
          </div>
        </div>
      `;
    } catch (err) {
      console.warn("[StreakWidget] Render error:", err);
    }
  }
}
